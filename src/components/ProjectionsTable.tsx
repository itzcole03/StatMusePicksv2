...
interface ProjectionsTableProps {
  ...
  onProjectionToggle: (id: string, checked: boolean) => void;
...
export default function ProjectionsTable({
  ...
  const groups: Group[] = (() => {
    ...
    for (const [key, items] of map.entries()) {
      ...
      for (const it of items) {
        ...
        sum += val;
        try {
          if (new Date(it.startTime).getTime() > new Date(latest.startTime).getTime()) latest = it;
        } catch (e) {
          // ignore
        }
      }
      ...
      out.push({ key, items, rep: latest, count: items.length, minLine: min, maxLine: max, avgLine: avg });
    }
    return out;
  })();
  ...
  const scheduleFetchForRange = useCallback((visibleProjs: ParsedProjection[]) => {
    // debounce rapid scroll events
    if (fetchTimer.current) clearTimeout(fetchTimer.current);
    fetchTimer.current = setTimeout(async () => {
      const toFetch = visibleProjs.filter(p => !fetchedIds.current.has(p.id));
      if (toFetch.length === 0) return;
      try {
        const ctxs = await buildExternalContextForProjections(toFetch as ParsedProjection[], settings as any);
  ...
  const OuterElement = React.forwardRef<HTMLDivElement, any>((props, ref) => {
    ...
    return (
      <div ref={ref} {...rest} className="px-6" style={{ ...(style || {}), overflowY: 'scroll' }}>
        {children}
      </div>
    );
  });

  const RowContainer = ({ children, style }: { children: any; style?: any }) => (
    <div className="grid items-center border-b border-gray-100 dark:border-gray-700" style={{ ...(style || {}), gridTemplateColumns: GRID_TEMPLATE, alignItems: 'center' }}>
      {children}
    </div>
  );
    ...
    return (
      <RowContainer style={style}>
        ...
        <div className="font-medium text-left flex items-center gap-2">
          ...
          {(proj.nbaContext?.noGamesThisSeason || nbaContexts[proj.id]?.noGamesThisSeason) && (
            <span title="No recent games this season" className="inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300">
              No recent
            </span>
          )}
        </div>
        ...
        <div className="text-left">
          <span className="px-2 py-1 text-xs font-semibold rounded-full bg-[#5D5CDE] bg-opacity-10 text-[#5D5CDE]">{proj.league}</span>
        </div>
        <div className="text-left">{proj.stat}</div>
        <div className="text-xl font-extrabold text-gray-900 dark:text-white text-right">{proj.line}</div>
    ...
    return (
      <RowContainer style={style}>
        ...
        <div className="ml-3 text-sm font-medium text-gray-200 dark:text-gray-300 flex items-center gap-3">
          ...
          <span>{group.minLine}/{group.maxLine}</span>
          <button onClick={() => setModalGroup(group)} className="text-sm px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded">View</button>
        </div>
      </RowContainer>
    );
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden mb-6">
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{showAll ? projections.length : groups.length} shown ({total} total)</span>
      </div>
      <div>
        <div className="px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setShowAll((s) => !s)} className="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded">
              {showAll ? 'Show collapsed' : 'Show all'}
            </button>
          </div>
        </div>
        {/* Column header to keep alignment with rows */}
        <div className="grid text-xs text-gray-400 dark:text-gray-300 px-6" style={{ gridTemplateColumns: GRID_TEMPLATE, alignItems: 'center' }}>
          <div />
        ...
        <div style={{ scrollbarGutter: 'stable' as any }}>
          <List
            height={Math.min(600, (showAll ? projections.length : groups.length) * ROW_HEIGHT)}
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-700 flex justify-center">
        {projections.length < total ? (
          <button onClick={loadMore} className="px-6 py-2 bg-[#5D5CDE] hover:bg-[#4a49c9] text-white rounded-lg">
            Load more
          </button>
        ) : (
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-xl w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Variants for {modalGroup.rep.player} — {modalGroup.rep.stat}</h3>
              <button onClick={() => setModalGroup(null)} className="text-gray-500">Close</button>
            </div>
            <div className="space-y-2 max-h-96 overflow-auto">
              {modalGroup.items.map((it) => (
                <div key={it.id} className="flex items-center px-3 py-2 border-b border-gray-100 dark:border-gray-700">
                  <input type="checkbox" checked={selectedProjections.has(it.id)} onChange={(e) => onProjectionToggle(it.id, e.target.checked)} className="mr-3" />
                  <div className="flex-1">
                    <div className="font-medium">{it.player} — {it.stat}</div>
                    <div className="text-sm text-gray-400 dark:text-gray-300">{it.team} • <span className="font-semibold text-gray-900 dark:text-white">{it.line}</span> • {new Date(it.startTime).toLocaleString()}</div>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-4 flex justify-end">
              <button onClick={() => setModalGroup(null)} className="px-4 py-2 bg-gray-100 dark:bg-gray-700 rounded">Done</button>
            </div>
          </div>
        </div>
      )}
