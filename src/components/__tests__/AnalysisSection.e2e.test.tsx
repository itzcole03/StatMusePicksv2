...
import { ParsedProjection } from '../../../src/types';
import { vi } from 'vitest';

// Mock the aiService helpers used by AnalysisSection
vi.mock('../../../src/services/aiService', async () => {
  return {
    buildAnalysisPromptAsync: vi.fn(async (projections: any[], settings: any) => {
    ...
    analyzeWithLocalLLM: vi.fn(async (prompt: string, settings: any, onChunk: (c: string) => void) => {
      ...
      const arr = settings.testProjections.map((p: any, idx: number) => ({
        ...
        numericEvidence: { recentGames: [12 + idx, 13 + idx, 11 + idx], seasonAvg: 12 + idx, opponent: { name: 'Opp' }, projectedMinutes: 30 },
    ...
    scoreModelOutput: (parsed: any[], projections: any[], contextsMap: Record<string, any> | null) => {
      ...
      const items = parsed.map((it, idx) => ({ index: idx, expectedRec: 'OVER', modelRec: it.recommendation, match: it.recommendation === 'OVER', heuristicScore: 80, modelScore: it.modelConfidenceScore }));
...
const sampleProjections = [
  ...
  { id: 'p2', player: 'Player Two', stat: 'points', line: 11, team: 'T2', league: 'NBA', startTime: Date.now() }
];
...
describe('AnalysisSection E2E compare LLM vs statistical model', () => {
  it('shows agreement between LLM and statistical model (aiService.v2)', async () => {
    ...
    const settings = { llmEndpoint: 'http://local.test', llmModel: 'test', requireExternalData: true, testProjections: sampleProjections } as any;

    render(<AnalysisSection projections={sampleProjections as any} settings={settings as any} />);

    // Wait for the agreement summary to appear
    await waitFor(() => expect(screen.getByText(/Model\/Heuristic agreement:/i)).toBeInTheDocument(), { timeout: 2000 });

    // The numeric '100%' might be wrapped in a <strong>, so assert the numeric token and headers separately
    expect(screen.getByText(/Model\/Heuristic agreement:/i)).toBeInTheDocument();
