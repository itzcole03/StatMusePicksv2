  ...
  return {
    ...
    analyzeWithLocalLLM: vi.fn(async (prompt: string, settings: any, onChunk: (c: string) => void) => {
      ...
      const arr = settings.testProjections.map((p: any, idx: number) => ({
        ...
        numericEvidence: { recentGames: [5 + idx, 6 + idx], seasonAvg: 5 + idx, opponent: { name: 'Opp' }, projectedMinutes: 25 },
    ...
    scoreModelOutput: (parsed: any[], projections: any[], contextsMap: Record<string, any> | null) => {
      const items = parsed.map((it: any, idx: number) => ({ index: idx, expectedRec: 'UNDER', modelRec: it.recommendation, match: it.recommendation === 'UNDER', heuristicScore: 20, modelScore: it.modelConfidenceScore }));
...
const sampleProjections = [
  { id: 'p1', player: 'Player One', stat: 'points', line: 12, team: 'T1', league: 'NBA', startTime: Date.now() }
];

describe('AnalysisSection E2E disagreement handling', () => {
  it('flags items for review when v2 strongly disagrees', async () => {
    const settings = { llmEndpoint: 'http://local.test', llmModel: 'test', requireExternalData: true, v2ConfidenceThreshold: 50, testProjections: sampleProjections } as any;
    ...
    await waitFor(() => expect(screen.getByText(/Items flagged for review are highlighted/i)).toBeInTheDocument(), { timeout: 2000 });
