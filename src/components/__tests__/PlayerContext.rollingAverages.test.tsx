  ...
  it("renders rolling averages chips when external context includes them", async () => {
    ...
    try {
      ...
      await screen.findByText(
        /External StatMuse data was fetched/i,
        undefined,
        { timeout: 3000 }
      );
      const showBtn = await screen
        .findByText(/Show details/i, undefined, { timeout: 2000 })
        .catch(() => null);
      if (showBtn) fireEvent.click(showBtn);

      // Check for rolling averages chip text after details are shown
      const chip = await screen
        .findByText(/last_3_avg: 21.00/i, undefined, { timeout: 2000 })
        .catch(() => null);
