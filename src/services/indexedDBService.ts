      ...
      try {
        ...
        if (count === 0) {
          ...
          const tx2 = db.transaction(
            [STORE, PLAYERS_STORE, "player_map"],
            "readwrite"
          );
          ...
          for (const name of playersSet) {
            try {
              tx2.objectStore(PLAYERS_STORE).put({ name });
            } catch {}
          }
          await tx2.done;
        }
...
export async function saveModelDirHandle(handle: any) {
  ...
  try {
    ...
    console.warn("saveModelDirHandle error", _err);
  }
}
  ...
  try {
    ...
    for (const [name, cnt] of batchCounts.entries()) {
      try {
        ...
        await tx
          .objectStore(PLAYERS_STORE)
          .put({ name, count: existingCount + cnt });
      } catch {
        // ignore single put errors
      }
    }
...
export async function rebuildPlayersIndex(): Promise<{
  ...
  try {
    ...
    for (const [name, cnt] of playerCounts.entries()) {
      try {
        await tx.objectStore(PLAYERS_STORE).put({ name, count: cnt });
      } catch {}
    }
    await tx.done;
...
export async function getDistinctPlayersByFilters(
  league?: string,
  stat?: string
) {
  const db = await initDB();
...
export async function getDistinctPlayersByFiltersWithQuery(
  league?: string,
  stat?: string,
  query?: string
) {
...
export async function getPlayerMappings(name: string) {
  ...
  try {
    ...
    for (const e of all) {
      if (!e || !e.name) continue;
      if (e.name === name)
        mappings.push({ league: e.league || "", stat: e.stat || "" });
    }
    // dedupe
...
export async function getPlayersWithCounts(
  league?: string,
  stat?: string,
  query?: string
) {
...
export async function queryProjections(
  filters: { league?: string; stat?: string; playerName?: string },
  offset = 0,
  limit = 50
) {
  const db = await initDB();
  ...
  const filtered = all.filter((value: ParsedProjection) => {
    ...
    if (
      filters.playerName &&
      filters.playerName.length &&
      !value.player.toLowerCase().includes(filters.playerName.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  try {
    for (const [id, ctx] of Object.entries(contexts)) {
      try {
        ...
        if (!mergedCtx.fetchedAt)
          mergedCtx.fetchedAt = new Date().toISOString();
