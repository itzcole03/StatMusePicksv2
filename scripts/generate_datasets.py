...
def main():
    ...
    p.add_argument('--seasons', type=str, default='2024-25', help='Comma-separated seasons list')
    ...
    p.add_argument('--db-only', action='store_true', help='Use only local DB (no network calls)')
    ...
    print('Querying DB for players with >=', args.min_games, 'games for stat', args.stat)
    ...
    try:
        ...
        q = text(f"SELECT id, full_name, fullName, display_name FROM players WHERE id IN ({placeholder})")
        ...
        id_to_name = {int(p.get('id')): p.get('full_name') or p.get('fullName') or p.get('display_name') for p in players_list}
    ...
    for pid in pids:
        ...
        try:
            ...
            if args.db_only:
                ...
                try:
                    df = tds.generate_training_data(name, stat=args.stat, seasons=seasons, min_games=args.min_games)
                except ValueError:
                    # retry without restricting to specific seasons (use all available history)
                    df = tds.generate_training_data(name, stat=args.stat, seasons=None, min_games=args.min_games, pid=pid)
                except Exception as e:
                    # Network or nba_api may be unavailable — fall back to building
                    # training rows from the local DB (player_stats + games join).
                    print('Warning: generate_training_data failed for', name, 'falling back to DB method:', str(e))
                    # Build from DB if possible
                    db_url_sync = _make_sync_db_url(db_url)
                    try:
                        from sqlalchemy import create_engine, text
                        eng = create_engine(db_url_sync)
                        q = text("SELECT g.game_date as game_date, ps.value as target FROM player_stats ps JOIN games g ON ps.game_id = g.id WHERE ps.player_id = :pid AND ps.stat_type = :stat ORDER BY g.game_date")
                        rows = []
                        with eng.begin() as conn:
                            res = conn.execute(q, {'pid': pid, 'stat': args.stat})
                            for r in res:
    ...
    train_df, val_df, test_df = tds.per_player_time_split(big_df, player_col='player', date_col='game_date')

    manifest = dataset_versioning.create_dataset_version(args.name, ','.join(seasons), train_df, val_df, test_df, output_dir=args.output_dir)
