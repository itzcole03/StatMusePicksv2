  ...
  for (const player of players) {
    try {
      ...
      const nbaRes = await fetch(
        `http://localhost:3000/api/nba/player_summary?player=${encodeURIComponent(
          player
        )}&stat=points&limit=5`
      );
      ...
      console.log(
        "NBA body sample:",
        JSON.stringify(nj, null, 2).slice(0, 1000)
      );
    } catch (e) {
    ...
    try {
      ...
      const smRes = await fetch(
        `http://localhost:3000/api/statmuse?player=${encodeURIComponent(
          player
        )}&stat=points&limit=5`
      );
      ...
      console.log(
        "StatMuse body sample:",
        JSON.stringify(sj, null, 2).slice(0, 1000)
      );
    } catch (e) {
...
if (typeof fetch === "undefined") {
  console.error(
    "Node version does not have global fetch. Use Node 18+ or run via a fetch polyfill."
  );
  process.exit(1);
}
