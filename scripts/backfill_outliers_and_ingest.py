...
def main():
    ...
    if not repaired:
        ...
        repaired_path = os.path.join(
            os.path.dirname(path), base.replace(".json", "_repaired.json")
        )
        ok = write_repaired(repaired_path, recs)
        if ok:
            print(
                f"Wrote repaired audit to {repaired_path} (repaired {len(repaired_idxs)} records)"
            )
        else:
        ...
        if subset:
            try:
                ...
                print(
                    f"update_player_stats inserted {inserted} rows for repaired subset"
                )
            except Exception as e:
