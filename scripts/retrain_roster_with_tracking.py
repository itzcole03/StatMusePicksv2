...
def run(reuse_seed: int | None = 0, save_models: bool = False, outpath: str | None = None):
    ...
    def add_tracking(df):
        ...
        trk_touches = np.clip(8 + 0.2 * df['target'] + rng.normal(0, 2, size=len(df)), 0, None)
        trk_poss = 100 + 0.5 * df['target'] + rng.normal(0, 10, size=len(df))
        trk_xfg = np.clip(0.4 + 0.005 * df['target'] + rng.normal(0, 0.03, size=len(df)), 0, 1)
        df = df.copy()
    ...
    if save_models:
        mr = ModelRegistry()
        try:
            mr.save_model('retrain_baseline_tmp', model_b, version='v-dryrun', notes='baseline from retrain_roster_with_tracking')
            mr.save_model('retrain_tracking_tmp', model_t, version='v-dryrun', notes='tracking from retrain_roster_with_tracking')
    ...
    recs_b = [{'pred_prob': float(p), 'actual': int(y>pred), 'odds': 2.0} for p, y, pred in zip(prob_b, y_test, pred_b)]
    recs_t = [{'pred_prob': float(p), 'actual': int(y>pred), 'odds': 2.0} for p, y, pred in zip(prob_t, y_test, pred_t)]

    df_b = pd.DataFrame(recs_b)
    df_t = pd.DataFrame(recs_t)

    engine = BacktestEngine(start_bankroll=1000.0)
    res_b = engine.run(df_b, prob_col='pred_prob', actual_col='actual', odds_col='odds', stake_mode='flat', flat_stake=5.0)
    res_t = engine.run(df_t, prob_col='pred_prob', actual_col='actual', odds_col='odds', stake_mode='flat', flat_stake=5.0)
    ...
    report = {
        ...
        'backtest_baseline': res_b.__dict__ if hasattr(res_b, '__dict__') else dict(res_b),
        'backtest_tracking': res_t.__dict__ if hasattr(res_t, '__dict__') else dict(res_t),
    }
...
def _cli():
    p = argparse.ArgumentParser(description='Retrain roster with synthetic tracking features (smoke)')
    p.add_argument('--seed', type=int, default=0, help='RNG seed for reproducibility (default: 0)')
    p.add_argument('--save-models', action='store_true', help='Persist temporary models to ModelRegistry')
    p.add_argument('--out', type=str, help='Output report path; defaults to models_store/backtest_reports')
    p.add_argument('--dry-run', action='store_true', help='Do not persist models; only run training and reports')
    args = p.parse_args()
    try:
        run(reuse_seed=args.seed, save_models=args.save_models and not args.dry_run, outpath=args.out)
    except SystemExit:
        raise
    except Exception:
