...
true_stat = 0.6 * last_5 + 0.2 * last_3 + 0.1 * (trk_xfg * 10) + 0.05 * (trk_touches) + noise
...
baseline = df.copy().drop(columns=['trk_avg_speed_m_s','trk_distance_m','trk_touches','trk_time_poss_sec','trk_exp_fg_pct'])
tracking = df.copy()
...
train_b, val_b, test_b = tds.chronological_split_by_ratio(baseline, date_col='game_date', train_frac=0.7, val_frac=0.15, test_frac=0.15)
train_t, val_t, test_t = tds.chronological_split_by_ratio(tracking, date_col='game_date', train_frac=0.7, val_frac=0.15, test_frac=0.15)
