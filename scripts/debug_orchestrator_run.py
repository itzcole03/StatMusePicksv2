...
def run():
    ...
    manifest = {"parts": {"train": {"files": {"features": str(t)}}, "val": {"files": {"features": str(v)}}, "test": {"files": {"features": str(te)}}}}
    manifest_path = work / "manifest.json"
    with open(manifest_path, "w", encoding="utf8") as fh:
        json.dump(manifest, fh)

    models_dir = work / "models"
    orchestrator_main(str(manifest_path), min_games=1, out_dir=str(models_dir), report_csv=str(work / "report.csv"), limit=None, workers=1, fit_calibrators=True)
    ...
    try:
        ...
        df_val["lag_3_mean"] = df_val["target"].shift(1).rolling(window=3, min_periods=1).mean().fillna(df_val["target"].mean())
        ...
        info = calib.fit_and_save("Test Player", y_true=y_val, y_pred=y_pred, method="isotonic")
