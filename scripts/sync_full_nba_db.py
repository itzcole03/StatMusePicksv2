...
def main():
    ...
    from backend.services import data_ingestion_service, nba_stats_client
    ...
    for season in seasons:
        ...
        if not season_map:
            logger.warning(
                "League-level season map empty for %s, falling back to per-player fetch (throttled)",
                season,
            )
            ...
            try:
                league_players = (
                    nba_stats_client.fetch_league_player_advanced(season) or {}
                )
            except Exception:
            ...
            logger.info(
                "Fallback will attempt up to %d league players (concurrent)", len(pids)
            )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as ex:
                ...
                for pid in pids:
                    try:
                        futures[
                            ex.submit(
                                nba_stats_client.fetch_full_player_history,
                                int(pid),
                                seasons=[season],
                            )
                        ] = pid
                        ...
                        logger.debug(
                            "Failed to submit fetch for pid=%s", pid, exc_info=True
                        )
                for fut in as_completed(futures):
                    ...
                    try:
                        ...
                        logger.exception(
                            "Failed to fetch games for player %s during fallback", pid
                        )
                        try:
                            # record failed fetch for later retry analysis
                            if hasattr(nba_stats_client, "_record_failed_fetch"):
                                nba_stats_client._record_failed_fetch(
                                    "player", pid, "fetch_full_player_history_fallback"
                                )
                        except Exception:
                            logger.debug(
                                "Failed to record failed fetch for %s",
                                pid,
                                exc_info=True,
                            )
                        continue
                    ...
                    for g in games:
                        ...
                        if matchup and isinstance(matchup, str):
                            if " @ " in matchup:
                                left, right = [
                                    t.strip() for t in matchup.split(" @ ", 1)
                                ]
                        ...
                        rec["home_team"] = (
                            home
                            or _normalize_team_name(
                                g.get("HOME_TEAM_ABBREVIATION")
                                or g.get("HOME_TEAM")
                                or g.get("home")
                            )
                            or "UNKNOWN"
                        ...
                        rec["away_team"] = (
                            away
                            or _normalize_team_name(
                                g.get("VISITOR_TEAM_ABBREVIATION")
                                or g.get("VISITOR_TEAM")
                                or g.get("away")
                            )
                            or "UNKNOWN"
                        )
        ...
        for pid, games in season_map.items():
            ...
            for g in games:
                ...
                if (not home or not away) and ("TEAM_ABBREVIATION" in g or "TEAM" in g):
                    ...
                    opp = (
                        g.get("OPPONENT")
                        or g.get("OPP_TEAM_ABBREVIATION")
                        or g.get("OPP_TEAM")
                        or g.get("OPPONENT_TEAM")
                    )
                    if team and opp:
                ...
                rec["home_team"] = (
                    home
                    or g.get("HOME_TEAM_ABBREVIATION")
                    or g.get("home")
                    or "UNKNOWN"
                )
                rec["away_team"] = (
                    away
                    or g.get("VISITOR_TEAM_ABBREVIATION")
                    or g.get("away")
                    or "UNKNOWN"
                )
                ...
                if len(batch) >= 1000:
                    ...
                    res = data_ingestion_service._process_and_ingest(
                        batch, when=None, dry_run=dry_run
                    )
                    if isinstance(res, dict):
                        total_player_rows += res.get("player_rows", 0)
                        missing_ct = len(res.get("validation", {}).get("missing", []))
                        type_err_ct = len(
                            res.get("validation", {}).get("type_errors", [])
                        )
                        ...
                        logger.info(
                            "Ingest validation: missing=%d type_errors=%d outliers=%d filtered_out=%d would_insert=%d",
                            missing_ct,
                            type_err_ct,
                            outliers_ct,
                            filtered_out,
    ...
    for t in teams:
        ...
        for g in games:
            ...
            if len(team_batch) >= 1000:
                logger.info(
                    "Flushing team batch of %d rows to ingestion", len(team_batch)
                )
                res = data_ingestion_service._process_and_ingest(
                    team_batch, when=None, dry_run=dry_run
                )
                if isinstance(res, dict):
                    missing_ct = len(res.get("validation", {}).get("missing", []))
                    type_err_ct = len(res.get("validation", {}).get("type_errors", []))
                    ...
                    logger.info(
                        "Team ingest validation: missing=%d type_errors=%d outliers=%d filtered_out=%d would_insert=%d",
                        missing_ct,
                        type_err_ct,
                        outliers_ct,
                        filtered_out,
