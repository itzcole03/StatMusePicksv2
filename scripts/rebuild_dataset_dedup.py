...
def main():
    ...
    # re-split
    train_df, val_df, test_df = tds.per_player_time_split(dedup, player_col='player', date_col='game_date')
    base_name = manifest.get('name','dataset') + '_' + args.name_suffix
    seasons = manifest.get('seasons','')
    out = dataset_versioning.create_dataset_version(base_name, seasons, train_df, val_df, test_df, output_dir=str(Path('backend/data/datasets')))
