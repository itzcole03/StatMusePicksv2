...
def compute_stats(train_df, val_df, test_df):
    ...
    out['target_quantiles'] = combined['target'].quantile([0.01,0.05,0.25,0.5,0.75,0.95,0.99]).to_dict()
    # per-player counts
    ...
    m = out['target_mean']; s = out['target_std']
...
def main():
    ...
    p.add_argument('--register-db', action='store_true', help='Insert metadata into DATABASE_URL')
    args = p.parse_args()
