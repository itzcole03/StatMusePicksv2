...
def delete_old_versions(
    name: str, keep: int = 1, output_dir: str = "datasets", dry_run: bool = True
) -> List[str]:
...
if __name__ == "__main__":
    ...
    p_prune.add_argument(
        "--yes", action="store_true", help="Perform deletion (otherwise dry-run)"
    )
    ...
    if args.cmd == "list":
        ...
        deleted = delete_old_versions(
            args.name, keep=args.keep, output_dir=args.output_dir, dry_run=not args.yes
        )
        if deleted:
