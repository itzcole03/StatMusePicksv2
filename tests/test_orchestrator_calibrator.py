...
def make_player_df(player_name: str, n: int, start_date: str = "2020-01-01") -> pd.DataFrame:
...
def test_orchestrator_trains_and_fits_calibrator(tmp_path):
    ...
    orchestrator_main(str(manifest_path), min_games=1, out_dir=str(models_dir), report_csv=str(report_csv), limit=None, workers=1, fit_calibrators=True)
