...
def upgrade():
    ...
    try:
        op.create_unique_constraint(
            "uq_model_metadata_name_version", "model_metadata", ["name", "version"]
        )
    except Exception:
        # If constraint creation fails (DB-specific), fall back to creating a unique index
        try:
            op.create_index(
                "uq_model_metadata_name_version_idx",
                "model_metadata",
                ["name", "version"],
                unique=True,
            )
...
def downgrade():
    try:
        op.drop_constraint(
            "uq_model_metadata_name_version", "model_metadata", type_="unique"
        )
    except Exception:
        try:
            op.drop_index(
                "uq_model_metadata_name_version_idx", table_name="model_metadata"
            )
        except Exception:
            pass
