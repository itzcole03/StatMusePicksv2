...
def _timescale_extension_present(conn) -> bool:
    try:
        r = conn.execute(sa.text("SELECT 1 FROM pg_extension WHERE extname='timescaledb' LIMIT 1;"))
...
def upgrade() -> None:
    ...
    if not _timescale_extension_present(conn):
        ...
        print("Timescale extension not found; skipping hypertable conversion for player_stats")
        return
    ...
    if force_flag != "1":
        try:
            ...
            print("player_stats table not found or unreadable; skipping hypertable conversion")
            return
