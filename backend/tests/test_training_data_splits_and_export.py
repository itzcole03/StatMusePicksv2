...
def _make_player_df(n_days=20, players=('A', 'B')):
    ...
    for p in players:
        for i in range(n_days):
            rows.append({'player': p, 'game_date': start + timedelta(days=i), 'feat': float(i)})
...
def test_per_player_time_split_preserves_chronology_and_counts():
    ...
    train, val, test = tds.per_player_time_split(df, player_col='player', date_col='game_date', train_frac=0.6, val_frac=0.2, test_frac=0.2)
...
def test_export_dataset_with_version_creates_manifest_and_files():
    ...
    with tempfile.TemporaryDirectory() as td:
        manifest = tds.export_dataset_with_version(df, y=y, output_dir=td, name='unit_ds', version='v0test', fmt_prefer='csv')
