...
def test_get_player_summary_playercareerstats_fallback(monkeypatch):
    ...
    out = nba_service.get_player_summary("Some Player", stat="points", limit=5, debug=False)
...
def test_get_player_summary_redis_errors_fallback(monkeypatch):
    ...
    out = nba_service.get_player_summary("Player X", stat="points", limit=1, debug=False)
