...
def test_chronological_split_by_ratio_counts_and_order():
    df, y = _make_df(20)
    train, val, test = tds.chronological_split_by_ratio(df, date_col='game_date', train_frac=0.7, val_frac=0.15, test_frac=0.15)
    assert len(train) == 14
    assert len(val) == 3
    assert len(test) == 3
...
def test_export_dataset_with_version_csv_fallback_and_manifest():
    ...
    with tempfile.TemporaryDirectory() as td:
        manifest = tds.export_dataset_with_version(df, y=y, output_dir=td, name='unittest', version='v0', fmt_prefer='csv')
