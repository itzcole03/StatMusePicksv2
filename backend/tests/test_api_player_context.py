...
def test_player_context_summary(monkeypatch):
    ...
    monkeypatch.setattr(app_mod, 'player_summary', lambda player, stat, limit, debug=0: {'player': player, 'stat': stat, 'recentGames': [], 'seasonAvg': None, 'fetchedAt': 'now'})

    client = TestClient(app)
    resp = client.post('/api/player_context', json={'player': 'LeBron James', 'stat': 'points', 'limit': 5})
...
def test_player_context_training_context(monkeypatch):
    ...
    sample_ctx = {'player': 'Player X', 'playerId': 7, 'season': '2023-24', 'recentGamesRaw': [], 'seasonStats': {}, 'advancedStats': {}, 'fetchedAt': 'now'}
    monkeypatch.setattr(svc, 'get_player_context_for_training', lambda player, stat, game_date, season: sample_ctx)

    client = TestClient(app)
    resp = client.post('/api/player_context', json={'player': 'Player X', 'season': '2023-24', 'game_date': '2024-01-01'})
    assert resp.status_code == 200
    data = resp.json()
