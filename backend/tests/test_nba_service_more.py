...
def test_playercareerstats_empty_frames(monkeypatch):
    ...
    monkeypatch.setattr(
        nba_service.nba_stats_client, "find_player_id_by_name", lambda name: 777
    )
    monkeypatch.setattr(
    ...
    out = nba_service.get_player_summary(
        "NoGames Player", stat="points", limit=3, debug=False
    )
...
def test_playercareerstats_raises_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        nba_service.nba_stats_client, "find_player_id_by_name", lambda name: 888
    )
    monkeypatch.setattr(
        nba_service.nba_stats_client,
        "fetch_recent_games",
    ...
    out = nba_service.get_player_summary(
        "Exploding PCS", stat="rebounds", limit=4, debug=False
    )
