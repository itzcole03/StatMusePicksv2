...
def test_end_to_end_calibration_alignment(tmp_path):
    ...
    y_train = X_train['f1'] * 2.0 + X_train['f2'] * -0.5 + rng.normal(scale=0.1, size=n_train)
    ...
    res = calib_service.fit_and_save('E2E Player', y_true=y_val, y_pred=raw_preds, method='isotonic')
