...
def test_team_advanced_from_cached_logs():
    ...
    assert pytest.approx(stats.get('PTS_diff', 0), rel=1e-3) == pytest.approx(3.5, rel=1e-3)

    # FG% = sum(FGM)/sum(FGA) = (45+42)/(85+88) = 87/173 ~= 0.50289
    assert pytest.approx(stats.get('FG_pct', 0), rel=1e-3) == pytest.approx(87.0 / 173.0, rel=1e-3)

    # FT% = 29/38 ~= 0.763157
    assert pytest.approx(stats.get('FT_pct', 0), rel=1e-3) == pytest.approx(29.0 / 38.0, rel=1e-3)
