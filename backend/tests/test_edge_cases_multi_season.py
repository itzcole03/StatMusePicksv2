...
def test_generate_training_data_handles_empty_season_stats(monkeypatch):
    ...
    # Resolve player id
    monkeypatch.setattr('backend.services.training_data_service.nba_stats_client.find_player_id_by_name', lambda name: 7)
    ...
    monkeypatch.setattr('backend.services.training_data_service.nba_stats_client.get_advanced_player_stats_multi', lambda pid, seasons: {'per_season': {}, 'aggregated': {}})
    # Season stats multi empty or missing PTS
    monkeypatch.setattr('backend.services.training_data_service.nba_stats_client.get_player_season_stats_multi', lambda pid, seasons: {s: {} for s in seasons})

    df = tds.generate_training_data('Edge Player', stat='points', min_games=10, fetch_limit=500, seasons=['2023-24', '2022-23'])
    ...
    assert 'multi_season_PTS_avg' in df.columns and all(df['multi_season_PTS_avg'] == 0.0)
