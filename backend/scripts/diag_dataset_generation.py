...
for f in sorted(Path('backend/data/datasets').rglob('dataset_manifest.json'), key=lambda x: x.stat().st_mtime, reverse=True)[:5]:
    print(f)
...
try:
    ...
    print('\nResolved', name, '->', pid)
    df = tds.generate_training_data(name, stat='points', min_games=1, fetch_limit=10, seasons=['2024-25'], pid=int(pid))
