...
def main(min_games: int, out_csv: str, limit: int | None):
    ...
    players = [
        r.get("full_name") for r in roster if r.get("is_active") and r.get("full_name")
    ]
    ...
    for idx, player in enumerate(players, start=1):
        ...
        try:
            df = training_data_service.generate_training_data(
                player, min_games=min_games, fetch_limit=500, seasons=seasons
            )
            rec["rows"] = int(len(df))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Per-player feature selection for active roster"
    )
    parser.add_argument("--min-games", type=int, default=5)
    parser.add_argument(
        "--out-csv", default="backend/models_store/feature_selection_report.csv"
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Optional limit on number of players to process",
    )
