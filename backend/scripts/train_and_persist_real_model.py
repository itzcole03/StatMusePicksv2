...
def build_dataset(n_samples=200):
    ...
    players = [p.strip() for p in players_env.split(',')] if players_env else ['LeBron James']
    ...
    for player in players:
        try:
            ...
            ctx = nba_service.get_player_context_for_training(player, stat, game_date=season, season=season)
            ...
            for i in range(1, len(recent)):
                ...
                if game_date and label is not None:
                    specs.append({'player': player, 'stat': stat, 'game_date': game_date, 'season': season, 'label': label})
        except Exception:
