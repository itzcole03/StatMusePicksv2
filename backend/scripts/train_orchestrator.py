    ...
    try:
        ...
        if fit_calibrator and val_df.shape[0] >= 3:
            try:
                ...
                cal_info = calib.fit_and_save(player, y_true=y_val, y_pred=y_pred_val, method="isotonic")
            except Exception:
                cal_info = None
        else:
            cal_info = None
...
def main(manifest: str, min_games: int, out_dir: str, report_csv: str, limit: int | None, workers: int = 1, fit_calibrators: bool = False, tune: bool = False, tune_trials: int = 20, feature_selection: bool = False):
    ...
    logger.info("Found %d players with >= %d rows (using limit=%s)", len(candidates), min_games, limit)
    ...
    with open(csv_path, "w", newline="", encoding="utf8") as fh:
        writer = csv.DictWriter(fh, fieldnames=["player", "train_rows", "val_rows", "test_rows", "val_mae", "test_mae", "model_path"]) 
        writer.writeheader()
...
if __name__ == "__main__":
    ...
    p.add_argument("--min-games", type=int, default=50, help="Minimum total rows for a player to train")
    p.add_argument("--out-dir", default="backend/models_store", help="Directory to save models")
    p.add_argument("--report-csv", default="backend/models_store/training_report.csv", help="CSV report path")
