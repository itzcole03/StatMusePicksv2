...
cur = conn.cursor()
try:
    cur.execute("SELECT player_id, COUNT(*) FROM player_stats GROUP BY player_id ORDER BY COUNT(*) DESC LIMIT 10")
    top10 = cur.fetchall()
    ...
    if row:
        top = row[0]
        cur.execute('SELECT * FROM player_stats WHERE player_id=? ORDER BY game_date LIMIT 5', (top,))
        sample = cur.fetchall()
