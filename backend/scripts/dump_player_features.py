...
def main(
    players: List[str], min_games: int = 1, fetch_limit: int = 500, seasons: str = None
):
    written = []
    for p in players:
        logger.info("Dumping features for %s", p)
        out = dump_player(
            p, min_games=min_games, fetch_limit=fetch_limit, seasons=seasons
        )
        if out:
            written.append(out)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Fetch limit passed to training_data_service",
    )
    parser.add_argument(
        "--seasons",
        default=None,
