...
def main():
    ...
    p.add_argument(
        "--out",
        default="backend/models_store/llm_samples.json",
        help="Output JSON file",
    )
    p.add_argument(
        "--players",
        default=None,
        help="Optional newline-separated file with player names",
    )
    p.add_argument(
        "--base-dir", default="backend/data/news_samples", help="Sample files base dir"
    )
    p.add_argument(
        "--dry-run", action="store_true", help="Only print results, don't write file"
    )
    args = p.parse_args()
    ...
    if args.players:
        players = [
            l.strip()
            for l in Path(args.players).read_text(encoding="utf-8").splitlines()
            if l.strip()
        ]
    else:
    ...
    for pl in players:
        text = fetcher(pl)
        features = svc.fetch_news_and_extract(
            pl, source_id="file_v1", text_fetcher=fetcher
        )
