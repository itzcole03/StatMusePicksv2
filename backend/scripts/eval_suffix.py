...
import pandas as pd
from backend.services.eval_report import load_model_if_exists, evaluate_model_on_df, write_report
import re
...
def find_model_with_suffix(models_dir: Path, player: str, suffix: str):
    ...
    for p in models_dir.glob(f"*{suffix}*.pkl"):
        if player.lower() in p.name.lower() or safe in re.sub(r'[^a-z0-9]', '_', p.name.lower()):
            return str(p)
    return None
...
def main(manifest: str, models_dir: str, suffix: str, out_csv: str):
    ...
    for player in players:
        ...
        if not model_path:
            rows.append({'player': player, 'status': 'missing_model', 'model_path': None})
            continue
        model = load_model_if_exists(model_path)
        if model is None:
            rows.append({'player': player, 'status': 'load_failed', 'model_path': model_path})
            continue
        ...
        if len(feature_cols) == 0:
            rows.append({'player': player, 'status': 'no_features', 'model_path': model_path})
            continue
        ...
        if 'metrics' in eval_res:
            ...
            rows.append({'player': player, 'status': 'error', 'error': eval_res.get('error'), 'model_path': model_path})
...
if __name__ == '__main__':
    ...
    p.add_argument('--suffix', required=True, help="Suffix string to match in filenames, e.g. '_abA' or '_abB'")
