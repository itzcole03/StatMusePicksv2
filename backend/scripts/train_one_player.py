...
def train_and_persist(player: str, stat: str = 'points'):
    ...
    X = df[['last_3_avg', 'last_5_avg', 'last_10_avg', 'last_3_std', 'days_rest', 'is_home', 'opp_def']]
    y = df['target']
    ...
    try:
        ...
        meta = {'player': player, 'stat': stat, 'rows': len(df), 'note': 'sklearn missing; no model trained'}
    ...
    meta = {'player': player, 'stat': stat, 'rows': len(df), 'model_path': str(model_path)}
    with open(out_dir / f"{safe_filename(player)}.meta.json", 'w') as f:
        json.dump(meta, f)
