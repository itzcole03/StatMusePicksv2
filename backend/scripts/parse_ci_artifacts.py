...
def summarize_alembic_log(path: Path):
    ...
    for i, l in enumerate(lines):
        ...
        if re.search(r"ERROR|Traceback|alembic.util.exc", l, re.I):
            ...
            errors.append(ctx)
        if re.search(
            r"skipp(ed|ing)|not supported|will be skipped|IF NOT EXISTS", l, re.I
        ):
...
def summarize_alembic_text(text: str, source: str = "<string>"):
    ...
    for i, l in enumerate(lines):
        ...
        if re.search(r"ERROR|Traceback|alembic.util.exc", l, re.I):
            ...
            errors.append(ctx)
        if re.search(
            r"skipp(ed|ing)|not supported|will be skipped|IF NOT EXISTS|Will assume non-transactional DDL",
            l,
            re.I,
        ):
            skipped.append(l.strip())
            ...
            if f.is_file():
                ...
                # detect embedded alembic output inside larger run logs
                    ...
                    try:
                        ...
                        if re.search(
                            r"Running upgrade|alembic.runtime.migration|Will assume non-transactional DDL",
                            text,
                            re.I,
                        ):
