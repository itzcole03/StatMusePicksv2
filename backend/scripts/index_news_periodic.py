...
OUT_PATH = os.path.join(
    os.path.dirname(__file__), "..", "artifacts", "vector_index.json"
)

SAMPLES = [
    {
        "id": "news:2001",
        "text": "Player A listed as questionable with ankle sprain.",
        "meta": {"player": "A"},
    },
    {
        "id": "game:20251123",
        "text": "Team B beat Team C 98-95; star scored 28 points.",
        "meta": {"type": "game"},
    },
]
...
def main():
    ...
    for s in SAMPLES:
        items_to_index.append(
            (s["id"], s["text"], {**s.get("meta", {}), "text": s["text"]})
        )
    ...
    for id in indexed:
        ...
        if hits:
            existing["items"].append(
                {"id": id, "meta": hits[0]["meta"], "indexed_at": now}
            )
