...
def find_metadata_files(base: Path):
    for p in base.rglob('*.json'):
        name = p.name.lower()
        if name.endswith('_metadata.json') or name == 'metadata.json' or 'metadata' in name:
            yield p
...
def extract_kept_from_json(p: Path):
    ...
    if 'metadata' in j and isinstance(j['metadata'], dict) and 'kept_contextual_features' in j['metadata']:
    ...
    for f in files:
        ...
        with eng.begin() as conn:
            ...
            if res.rowcount and res.rowcount > 0:
                ...
                print(f"Updated {res.rowcount} rows for {name} version={version} from {f}")
