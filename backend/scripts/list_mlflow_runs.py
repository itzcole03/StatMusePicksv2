...
for exp in exps:
    ...
    runs = client.search_runs(exp.experiment_id, order_by=["attributes.start_time DESC"], max_results=20)
    if not runs:
        print('  No runs for this experiment')
    for r in runs:
        info = r.info
        st = datetime.fromtimestamp(info.start_time / 1000.0) if info.start_time else None
