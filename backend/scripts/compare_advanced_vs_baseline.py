...
def evaluate_player(player: str, min_games: int = 1) -> dict:
    try:
        df = training_data_service.generate_training_data(player, min_games=min_games, fetch_limit=300)
    ...
    try:
        train_df, val_df, test_df = training_data_service.chronological_split_by_ratio(df_adv, date_col='game_date', train_frac=0.7, val_frac=0.15, test_frac=0.15)
    except Exception:
    ...
    try:
        model_base = training_pipeline.train_player_model(train_base, target_col='target')
    except Exception:
    ...
    def prepare_X(df_source, train_reference):
        X = df_source.drop(columns=['target'], errors='ignore').select_dtypes(include=[np.number]).copy()
        # align columns to training X
        train_cols = list(train_reference.drop(columns=['target'], errors='ignore').select_dtypes(include=[np.number]).columns)
        for c in train_cols:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compare advanced vs baseline models for a roster')
