...
def train_and_save(player: str, output_dir: str = "backend/models_store") -> str:
    try:
        ...
        df = training_data_service.generate_training_data(player, min_games=min_games, fetch_limit=300, seasons=seasons)
    except Exception:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Retrain small roster with advanced + LLM features (smoke)")
    parser.add_argument("--players", help="Comma-separated player list (defaults to full NBA roster)", default=None)
    parser.add_argument("--min-games", type=int, default=5, help="Minimum games required to generate training data")
    ...
    if args.players:
        ...
        try:
            ...
            for p in allp:
                ...
                name = p.get('full_name') or p.get('fullName') or p.get('display_name') or p.get('displayName')
                if not name:
                    continue
                is_active = False
    ...
    if now.month >= 10:
        ...
        start_year = year - 1
    seasons_list = [f"{start_year - i}-{str((start_year - i + 1)%100).zfill(2)}" for i in range(0, 3)]
