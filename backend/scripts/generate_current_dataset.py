...
def build_dataset_for_roster(
    ...
    if resume and progress_path and progress_path.exists():
        try:
            ...
            logger.info(
                "Resuming dataset generation; %d players already processed",
                len(processed),
            )
        except Exception:
    ...
    for idx, name in enumerate(roster, start=1):
        ...
        for attempt in range(3):
            try:
                ...
                logger.debug(
                    "find_player_id_by_name failed for %s (attempt %d): %s",
                    name,
                    attempt + 1,
                    exc,
                )
        ...
        if not name or not pid:
            logger.info(
                "Skipping roster name without resolvable id: %s (%d/%d)",
                name,
                idx,
                total,
            )
        ...
        for attempt in range(3):
            q = mp.Queue()
            p = mp.Process(
                target=_per_player_worker,
                args=(q, name, pid, seasons, min_games, fetch_limit),
            )
            p.start()
            p.join(timeout=getattr(build_dataset_for_roster, "_per_player_timeout", 60))
            if p.is_alive():
                logger.warning(
                    "Player generation timed out for %s (attempt %d)", name, attempt + 1
                )
                try:
                    p.terminate()
                except Exception:
            ...
            if res.get("ok"):
                ...
                logger.debug(
                    "Attempt %d failed for %s: %s", attempt + 1, name, res.get("error")
                )
    ...
    if "player_id" not in combined.columns:
        logger.info(
            "player_id column missing from generated rows; attempting to resolve via name"
        )
    ...
    try:
        ...
        combined["player_id"] = combined["player_id"].apply(
    ...
    manifest_info = dv.create_dataset_version(
        name="points_dataset",
        seasons=",".join(seasons),
        df_train=train_df,
        df_val=val_df,
        df_test=test_df,
        output_dir="backend/data/datasets",
    )
    ...
    try:
        ...
        for split in ("train", "val", "test"):
            ...
            if "player_id" not in cols:
                logger.warning(
                    "Exported manifest part %s missing player_id column; consider re-running with parquet engine available",
                    split,
                )
    except Exception:
...
def main():
    ...
    p.add_argument(
        "--seasons", default="2024-25", help="Comma-separated seasons (e.g. 2024-25)"
    )
    ...
    p.add_argument(
        "--resume",
        action="store_true",
        help="Resume from previous progress file if present",
    )
    p.add_argument(
        "--progress-file",
        default="backend/models_store/dataset_generation_progress.json",
        help="Path to write progress/resume file",
    )
    args = p.parse_args()
    setup_logging(verbose=getattr(args, "verbose", False))
    seasons = [s.strip() for s in args.seasons.split(",") if s.strip()]
    info = build_dataset_for_roster(
        seasons,
