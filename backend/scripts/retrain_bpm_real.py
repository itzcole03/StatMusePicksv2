...
def find_latest_points_manifest():
    pattern = os.path.join('backend', 'data', 'datasets', 'points_dataset_*', 'dataset_manifest.json')
...
def evaluate(df, name):
    X = df.drop(columns=['target', 'player', 'game_date'], errors='ignore').select_dtypes(include=[np.number]).fillna(0)
...
def main():
    ...
    report = pd.DataFrame([
        {'experiment': 'baseline_real', 'rmse_cv': rmse_base, 'model_path': path_base},
        {'experiment': 'with_bpm_real', 'rmse_cv': rmse_bpm, 'model_path': path_bpm},
    ])
    ...
    with open(os.path.join(OUT_DIR, 'retrain_bpm_real_summary.json'), 'w', encoding='utf-8') as fh:
