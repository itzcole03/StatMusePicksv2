...
def map_roster_to_manifest(manifest_path: str, roster: list) -> dict:
    ...
    for rname in roster:
        ...
        if not found:
            ...
            matches = [
                orig
                for k_norm, orig in norm_index.items()
                if nn and (nn in k_norm or k_norm in nn)
            ]
            if len(matches) == 1:
                found = matches[0]
    ...
    if not has_player_id:
        ...
        with open(mismatch_csv, "w", newline="", encoding="utf8") as fh:
            writer = csv.DictWriter(
                fh, fieldnames=["roster_name", "resolved_player_id"]
            )
            writer.writeheader()
            for r in rows:
                writer.writerow(r)
        ...
        logger.info(
            "Manifest features missing `player_id`; wrote mismatch CSV to %s",
            mismatch_csv,
        )
        logger.info(
            "Recommend regenerating dataset manifest with `player_id` column before running roster training."
        )
        return
