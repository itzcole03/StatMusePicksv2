...
def main(manifest: str, models_dir: str, out_csv: str = None):
    ...
    if wpath.exists():
        try:
            ...
            for _, r in wdf.iterrows():
                if 'player' in r and 'best_weights' in r and pd.notna(r['best_weights']):
    ...
    for player in players:
        ...
        if not path_a or not path_b:
            rows.append({'player': player, 'status': 'missing_models', 'path_a': path_a, 'path_b': path_b})
            continue
        ...
        rows.append({'player': player, 'status': 'ok', 'rmse': rmse, 'mae': mae, 'w_used': str(w)})
