"""Elastic Net training wrapper using scikit-learn ElasticNetCV.

Provides: train_elastic_net, predict, get_coefs, save_model, load_model
"""
...
def train_elastic_net(X, y, l1_ratio=(0.1, 0.5, 0.9), alphas=None, cv=3, random_state=0):
    """Train ElasticNetCV and return fitted model."""
    model = ElasticNetCV(l1_ratio=l1_ratio, alphas=alphas, cv=cv, random_state=random_state)
    model.fit(X, y)
    return model
