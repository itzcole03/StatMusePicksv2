...
class ModelRegistry:
        ...
        try:
            ...
            with engine.begin() as conn:
                ...
                ins_kwargs = dict(
                    name=player_name,
                    version=version,
                    path=os.path.abspath(versioned_path),
                    notes=notes,
                )
                    ...
                    if run is not None and getattr(run.info, 'run_id', None):
                        ...
                        try:
                            if ins_kwargs.get('notes') is None:
                                ...
                                try:
                                    parsed = json.loads(ins_kwargs['notes']) if isinstance(ins_kwargs['notes'], str) else ins_kwargs['notes']
                ...
                try:
                    ...
                    if expected is not None:
                        ...
                        if actual is None or actual != expected:
                            raise RuntimeError('Artifact signature mismatch for %s' % player_name)
                except Exception:
                    logger.exception('Artifact signature verification failed for %s', player_name)
                    raise
        ...
        try:
            ...
            for name in os.listdir(self.model_dir):
                ...
                if os.path.isdir(p):
                    ...
                    items.add(name + '.pkl')
                elif os.path.isfile(p) and name.endswith('.pkl') and not name.endswith('_calibrator.pkl'):
                    items.add(name)
            return sorted(items)
        except Exception:
            return []
    ...
    def validate_feature_list(self, player_name: str, feature_list: list) -> bool:
            ...
            try:
                ...
                if os.path.isdir(versions_dir):
                    for root, dirs, files in os.walk(versions_dir):
                        for f in files:
                            if f == 'model.pkl':
                                sidecar_paths.append(os.path.splitext(os.path.join(root, f))[0] + '_metadata.json')
            except Exception:
                pass
            # legacy sidecar
            legacy_sidecar = os.path.splitext(self._model_path(player_name))[0] + '_metadata.json'
        ...
        try:
            ...
            engine = create_engine(sync_url, future=True)
            with engine.begin() as conn:
                sel = ModelMetadata.__table__.select().where(ModelMetadata.__table__.c.name == player_name)
                row = conn.execute(sel).first()
                if row is not None:
                    sc = row.get('feature_list_checksum') if isinstance(row, dict) else getattr(row, 'feature_list_checksum', None)
                    if sc is not None:
                        return sc == provided_checksum
        except Exception:
            pass
            ...
            if os.path.exists(sidecar) and os.environ.get('MODEL_ARTIFACT_SIGNING_KEY'):
                try:
                    ...
                    if expected is not None:
                        ...
                        if actual is None or actual != expected:
                            raise RuntimeError('Calibrator artifact signature mismatch for %s' % player_name)
                except Exception:
                    logger.exception('Calibrator signature verification failed for %s', player_name)
                    raise
