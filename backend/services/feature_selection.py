...
def select_by_correlation(df: pd.DataFrame, target_col: str = "target", thresh: float = 0.01) -> List[str]:
...
def rfe_select(df: pd.DataFrame, target_col: str = "target", n_features: Optional[int] = None, estimator=None, step: int = 1) -> List[str]:
    ...
    n_features = int(n_features) if n_features is not None else max(1, min(10, X.shape[1] // 2))
