...
def evaluate_model_on_df(model, df: pd.DataFrame, feature_cols: list, target_col: str = 'target') -> Dict[str, Any]:
    if df.shape[0] == 0:
        ...
        try:
            ...
            if expected_n and expected_n > 0:
                try:
                    ...
                    return {"error": "predict_failed", "exception": str(e2), "X_shape": X.shape, "expected_n": expected_n}
            else:
                return {"error": "predict_failed", "exception": str(e2), "X_shape": X.shape}
