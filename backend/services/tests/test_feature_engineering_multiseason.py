...
def test_engineer_features_includes_multi_season_fields():
    ...
    player_context = {
        ...
        "seasonAvg": 14.0,
        "advancedStatsMulti": {
            "aggregated": {"PER": 13.5, "TS_PCT": 0.55, "USG_PCT": 22.0}
        },
    ...
    assert "multi_off_rating" in df.columns and isinstance(
        df.iloc[0]["multi_off_rating"], float
    )
    assert "multi_def_rating" in df.columns and isinstance(
        df.iloc[0]["multi_def_rating"], float
    )
