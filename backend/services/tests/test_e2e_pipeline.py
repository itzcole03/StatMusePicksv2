...
def test_e2e_context_to_features_and_training(monkeypatch):
    ...
    monkeypatch.setattr(client, 'fetch_recent_games', lambda pid, lim, season: games[:lim])
    monkeypatch.setattr(client, 'fetch_recent_games_multi', lambda pid, seasons, limit_per_season: games)
    monkeypatch.setattr(client, 'get_player_season_stats', lambda pid, season: {'PTS': 15.0})
    monkeypatch.setattr(client, 'get_advanced_player_stats', lambda pid, season: {'PER': 12.5, 'TS_PCT': 0.56, 'USG_PCT': 21.0, 'PIE': 9.5, 'OFF_RATING': 105.0, 'DEF_RATING': 99.0})
    monkeypatch.setattr(client, 'get_player_season_stats_multi', lambda pid, seasons: {s: {'PTS': 14.0 + i} for i, s in enumerate(seasons)})
    monkeypatch.setattr(client, 'get_advanced_player_stats_multi', lambda pid, seasons: {'per_season': {s: {'PER': 11.0 + i} for i, s in enumerate(seasons)}, 'aggregated': {'PER': 11.5, 'TS_PCT': 0.55}})
    monkeypatch.setattr(client, 'get_team_stats_multi', lambda tid, seasons: {s: {'PTS_avg': 110 + i} for i, s in enumerate(seasons)})
    monkeypatch.setattr(client, 'get_advanced_team_stats_multi', lambda tid, seasons: {'per_season': {s: {'OFF_RATING': 104 + i} for i, s in enumerate(seasons)}, 'aggregated': {'OFF_RATING': 105.0}})

    # 1) get_player_context_for_training
    ctx = svc.get_player_context_for_training('Any Player', 'points', '2023-02-01', '2023-24')
    ...
    train_df = tds.generate_training_data('Any Player', stat='points', min_games=5, fetch_limit=100, seasons=['2021-22', '2022-23'])
