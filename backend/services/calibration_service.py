...
class CalibrationService:
    def __init__(self, model_dir: Optional[str] = None):
        ...
        self.registry = (
            ModelRegistry(model_dir=model_dir) if model_dir else ModelRegistry()
        )

    def fit_calibrator(
        self, y_true: np.ndarray, y_pred: np.ndarray, method: str = "isotonic"
    ):
        ...
        if len(y_true) != len(y_pred) or len(y_true) < 3:
            raise ValueError(
                "Need at least 3 paired predictions/targets to fit calibrator"
            )
    ...
    def fit_and_save(
        ...
        try:
            brier_before = float(
                brier_score((y_true > 0).astype(float), np.asarray(y_pred))
            )
            ece_before = float(
                expected_calibration_error(
                    (y_true > 0).astype(float), np.asarray(y_pred)
                )
            )
        ...
        try:
            brier_after = float(
                brier_score((y_true > 0).astype(float), np.asarray(y_cal))
            )
            ece_after = float(
                expected_calibration_error(
                    (y_true > 0).astype(float), np.asarray(y_cal)
                )
            )
        ...
        try:
            ...
            report_path = os.path.join(
                reports_dir, f"{safe}_calib_report_{method}_{ts}.json"
            )
            report = {
                "player": player_name,
                "method": method,
                "created_at": ts,
                "calibrator_path": (
                    os.path.abspath(self.registry._calibrator_path(player_name))
                    if hasattr(self.registry, "_calibrator_path")
                    else None
                ),
                "before": before,
                "after": after,
        ...
        try:
            ...
            notes = json.dumps(
                {"calibration_method": method, "before": before, "after": after}
            )
            ...
            logger.exception(
                "Failed to persist ModelMetadata for calibrator %s", player_name
            )
